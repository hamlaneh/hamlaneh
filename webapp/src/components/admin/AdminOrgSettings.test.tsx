...
const SETTINGS: OrgSettings = {
  ...
  encrypted_conversations: 0,
  plaintext_conversations: 0,
};
...
describe("the organization encryption mode", () => {
  ...
  it("states the standing count of conversations outside the mode", async () => {
    live = { ...SETTINGS, plaintext_conversations: 12 };
    renderScreen();
  ...
  it("confirms a switch to Strict, and says what it does not do to what exists", async () => {
    ...
    live = {
      ...
      encrypted_conversations: 3,
      plaintext_conversations: 7,
    };
    renderScreen();
    ...
    // and it names what will be outside the mode being CHOSEN. Choosing
    // Strict, that is the seven this server can still read, not the three it
    // already cannot: telling an admin about the encrypted ones here would
    // answer a question they are not asking and hide the one that matters.
    expect(
      within(dialog).getByText(ENCRYPTION.outside.strict.replace("{{conversations}}", "7")),
    ).toBeInTheDocument();
    expect(
      within(dialog).queryByText(ENCRYPTION.outside.compliance.replace("{{conversations}}", "3")),
    ).toBeNull();
...
describe("the switch dialog's other direction", () => {
   ...
   */
  function renderComplianceDialog(outsideTarget = 0) {
    render(
      <EncryptionModeSwitchDialog
        to="compliance"
        conversationsOutsideTarget={outsideTarget}
        busy={false}
        error={null}
        onConfirm={() => undefined}
        onCancel={() => undefined}
  ...
  it("shows the live count before confirming", () => {
    ...
    // This dialog chooses Compliance, so the count it names is the encrypted
    // conversations Compliance will never reach — search, retention and export
    // stop at them permanently, which is the thing worth knowing before
    // choosing a mode whose whole purpose is those three.
    expect(
      screen.getByText(ENCRYPTION.outside.compliance.replace("{{conversations}}", "7")),
    ).toBeInTheDocument();
  });
});
