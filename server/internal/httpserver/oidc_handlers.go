...
// owns the transaction cookie, the linking state machine, and the fact that
// an SSO sign-in ends in the SAME session mint as a password sign-in, so
// the org two-step policy and session lifetime bind to it unchanged.
...
// completeOidcLogin finishes a sign-in intent: the identity must already be
// linked to an account (just-in-time provisioning does not exist in this
// slice — deliberately, so "SSO cannot bypass registration-off" is true by
// construction), and the account then goes through the SAME session mint as
// a password sign-in: advisory lock, org session lifetime, the two-step
// enrolment flag decided at mint, and the audit record.
func (s *apiServer) completeOidcLogin(w http.ResponseWriter, r *http.Request, store Store, ident oidc.Identity) {
	...
	if errors.Is(err, storage.ErrNotFound) {
		s.refuseUnknownIdentity(w, r, store, ident)
		return
	}
	if err != nil {
...
// refuseUnknownIdentity answers an identity linked to nobody. The email
// claim is consulted for exactly one thing: PHRASING the refusal —
// sso_account_exists when it names a local account (which links from
// Settings while signed in; auto-linking it here would fuse the weakest
// email assertion on either side into a session, ADR 004), and
// sso_account_unknown otherwise. Nothing is created and nobody signs in on
// either branch.
func (s *apiServer) refuseUnknownIdentity(w http.ResponseWriter, r *http.Request, store Store, ident oidc.Identity) {
	if ident.Email != "" {
		_, err := store.UserByEmail(r.Context(), ident.Email)
		switch {
		case err == nil:
			ssoFail(w, r, codeSSOAccountExists)
			return
		case !errors.Is(err, storage.ErrNotFound):
			slog.Error("sso refusal email lookup", "error", err)
			ssoFail(w, r, codeSSOFailed)
			return
		}
	}
...
func (s *apiServer) UnlinkOidcIdentity(w http.ResponseWriter, r *http.Request) {
	...
	// password-less SSO/SCIM accounts slice 3's migration will create.
	// users.password_hash is still NOT NULL today (migration 0001), so no
	// such account exists yet; the guard is here so unlinking cannot strand
	// one the moment they do. Checked after the linked check: an account
	// with neither a password nor a link gets the honest "nothing to unlink".
